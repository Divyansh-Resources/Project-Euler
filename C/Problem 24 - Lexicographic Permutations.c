...
void get_representation(long, int[], int[]);
//void display_representation(long, int[]);
void convert_representation_to_tuple(int[], int[]);
void display_tuple(int, int[]);

int main()
{
    int hash_table_fact[no_of_tuple_elements], representation[no_of_tuple_elements], tuple[no_of_tuple_elements];
    long number = question - 1;//There is a zeroeth permutation. Permutation #n corresponds with #(n-1) in the new representation

    create_hash_table(hash_table_fact); //Where T[n] = n!

    get_representation(number, representation, hash_table_fact);
    //display_representation(number, representation);
    convert_representation_to_tuple(representation, tuple);
...
void get_representation(long number, int representation[], int hash_table_fact[])
{
    int i = no_of_tuple_elements - 1;//Counting starts from 0
    long current_digit = 0, current_place_value;

    //Finds the ith digit of the number in a new representation
    for(i = no_of_tuple_elements - 1; i >= 0; i--)
    {
        current_digit = 0;
        current_place_value = hash_table_fact[i]; //Place value in this new representation is i!
        ...
        //Updating the value of current digit in representation
        representation[i] = current_digit;
        number = number - current_digit*current_place_value; //Updating the value of number for the next iteration
    }
}

/*//Displaying the number's new representation - Only for testing. Not required
void display_representation(long number, int representation[])
{
    int i;
...
void convert_representation_to_tuple(int representation[], int tuple[])
{
    ...
    for(i = no_of_tuple_elements - 1; i >= 0; i--)
    {
        current_digit = representation[i];
