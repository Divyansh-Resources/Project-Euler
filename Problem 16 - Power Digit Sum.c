#include <stdio.h>
#define size_of_number 330 //2^1000 < 8^333 < 10^333. So I've set the size at 330
...
void find_power(int[],int);

int main()
{
    int number[size_of_number], sum, num = 2;

    initialise(number);

    find_power(number, num);
...
void find_power(int number[], int num)
{
    int i, first_digit;
